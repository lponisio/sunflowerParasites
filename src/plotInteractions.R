...
plotInteractions <- function(){
    ...
    ## top historgram
    h <- hist(sp.by.site$FloralRichness, breaks=nbreaks, plot=FALSE)
    ...
    h.hedgerow <-
        hist(sp.by.site$FloralRichness[sp.by.site$Site %in% site.hr],
             breaks=h$breaks, plot=FALSE)
    plot(h.hedgerow,
         xlab="", main="", ylab="", yaxy="n", xaxt="n", add=TRUE, density=15)
    abline(v=mean(sp.by.site$FloralRichness), lty=2, col="red", lwd=3)
    ...
    ## real.mids <- h$mids
    abund.hist.mids <- (real.mids - mean(sp.by.site$FloralRichness))/
        sd(sp.by.site$FloralRichness)

    mtext("Frequency", 2, line=4, cex=1.3)
    mtext("Non-crop floral richness", 1, line=3, cex=1.3)
    ...
    for(i in 1:length(abund.hist.mids)){
        curve(inv.logit(mod.coeffs["(Intercept)"] +
                        ...
                        mod.coeffs["scale(FloralRichness)"] * abund.hist.mids[i] +
                        mod.coeffs["scale(TotalAbundance):scale(FloralRichness)"] * x * abund.hist.mids[i]),
       ...
       curve(inv.logit(mod.coeffs["(Intercept)"] +
                        mod.coeffs["scale(TotalAbundance)"] * x +
                        mod.coeffs["scale(FloralRichness)"] * 0 +
                        mod.coeffs["scale(TotalAbundance):scale(FloralRichness)"] * x * 0),
              from=range(sp.by.site$TotalAbundance)[1],
              to=range(sp.by.site$TotalAbundance)[2],
             col="red",
             lty="dashed",
